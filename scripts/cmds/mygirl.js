...
module.exports.onStart = async ({ event, api, args }) => {
  ...
  try {
    ...
    if (!mention)
      return api.sendMessage("Por favor, marque ou responda a 1 pessoa", threadID, messageID);
    ...
    api.sendMessage({
      body: `𝐄𝐒𝐒𝐀 𝐄́ 𝐌𝐈𝐍𝐇𝐀 𝐌𝐄𝐍𝐈𝐍𝐀 🖤`,
      attachment: fs.createReadStream(imgPath)
    }, threadID, () => fs.unlinkSync(imgPath), messageID);

  } catch (error) {
    console.error(error);
    api.sendMessage("🥹erro, contate MahMUD.", event.threadID, event.messageID);
  }
};